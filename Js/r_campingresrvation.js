...
var camslides = document.querySelectorAll('.campslide');
...
function manButtonNav(index) {
    for(var i = 0; i < camslides.length; i++) {
        if(i !== index) {
            camslides[i].classList.remove('sli');
            radiobtn[i].classList.remove('active');
        }
        else {
            camslides[index].classList.add('sli');
...
function prevSlide() {
    ...
    if(sli.previousElementSibling) {
        ...
        camslides[camslides.length - 1].classList.add('sli');
