...
var jeepslide = document.querySelectorAll('.jeepslide');
...
function manButtonNav(index) {
    for(var i = 0; i < jeepslide.length; i++) {
        if(i !== index) {
            jeepslide[i].classList.remove('sli');
            radiobtn[i].classList.remove('active');
        }
        else {
            jeepslide[index].classList.add('sli');
...
function prevSlide() {
    ...
    if(sli.previousElementSibling) {
        ...
        jeepslide[jeepslide.length - 1].classList.add('sli');
